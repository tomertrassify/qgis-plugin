...
FORM_CLASS, _ = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "editor_widget_wms.ui")
)
...
class EditorWidgetWms(QWidget, FORM_CLASS):
    def __init__(self, parent=None):
        ...
        self.setupUi(self)
        self.wms_validator = LineEditColorValidator(
    ...
    def feel_form(self, ds_info):
        ...
        self.txtParams.setText(
            ds_info.wms_params + "&" + ds_info.wms_url_params
        )
        self.txtLayers.setText(ds_info.wms_layers)
        self.chkTurnOver.setChecked(
    ...
    def validate(self, ds_info):
        ...
        if not self.wms_validator.is_valid():
            QMessageBox.critical(
                self,
                self.tr("Error on save data source"),
                self.tr("Please, enter correct value for WMS url"),
            )
            return False
