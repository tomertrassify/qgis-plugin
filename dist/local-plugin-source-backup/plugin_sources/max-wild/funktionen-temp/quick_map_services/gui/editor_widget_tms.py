...
FORM_CLASS, _ = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "editor_widget_tms.ui")
)
...
class EditorWidgetTms(QWidget, FORM_CLASS):
    ...
    def feel_form(self, ds_info):
        ...
        self.spbZMin.setValue(
            int(ds_info.tms_zmin) if ds_info.tms_zmin else self.spbZMin.value()
        )
        self.spbZMax.setValue(
            int(ds_info.tms_zmax) if ds_info.tms_zmax else self.spbZMax.value()
        )
        self.chkOriginTop.setChecked(
            True
            if (
                ds_info.tms_y_origin_top is None
                or ds_info.tms_y_origin_top == 1
            )
            else False
        )
        ...
        if self.rbCrsId.isChecked():
            try:
                ...
                QMessageBox.critical(
                    self,
                    self.tr("Error on save data source"),
                    self.tr("Please, enter correct CRC ID"),
                )
                return False
