...
class GroupEditDialog(QDialog, FORM_CLASS):
    ...
    def set_icon(self, icon_path):
        ...
        self.iconPreview.setPixmap(QPixmap(self.__group_icon))
    ...
    def validate(self, group_id, group_alias, group_icon):
        ...
        for val, comment in checks:
            if not val:
                QMessageBox.critical(
                    self, self.tr("Error on save group"), comment
                )
                return False
        ...
        for val, comment in checks_correct:
            if not val.is_valid():
                QMessageBox.critical(
                    self, self.tr("Error on save group"), self.tr(comment)
                )
                return False
    ...
    def save_existing(self):
        ...
        if group_id != self.group_info.id and not self.check_existing_id(
            group_id
        ):
            return False
        ...
        if (
            ...
            and group_alias == self.group_info.alias
            and is_same(group_icon, self.group_info.icon)
        ):
            return True
        ...
        with codecs.open(
            self.group_info.file_path, "w", "utf-8"
        ) as configfile:
