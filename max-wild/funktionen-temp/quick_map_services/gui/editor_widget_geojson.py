...
FORM_CLASS, _ = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "editor_widget_geojson.ui")
)
...
class EditorWidgetGeoJson(QWidget, FORM_CLASS):
    def __init__(self, parent=None):
        ...
        self.setupUi(self)
        self.geojson_validator = LineEditColorValidator(
    ...
    def validate(self, ds_info):
        if not ds_info.geojson_url:
            QMessageBox.critical(
                ...
                self.tr("Please, enter GeoJSON url"),
            )
            return False
        ...
        if not self.geojson_validator.is_valid():
            QMessageBox.critical(
                self,
                self.tr("Error on save data source"),
                self.tr("Please, enter correct value for GeoJSON url"),
            )
            return False
